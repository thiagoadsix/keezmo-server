...
describe('FindDeckStatsUseCase', () => {
  ...
  const mockDueProgress = mockProgressRecords.filter(
    (p): p is Progress => p !== null && ['card-4', 'card-5', 'card-6', 'card-7'].includes(p.cardId)
  );
  ...
  beforeEach(() => {
    ...
    vi.mocked(progressRepository.findByCardAndDeck).mockImplementation((cardId: string) => {
      const index = Number(cardId.split('-')[1]) - 1;
      return Promise.resolve(mockProgressRecords[index]);
    });
    vi.mocked(progressRepository.findDueCards).mockResolvedValue(mockDueProgress);

    sut = new FindDeckStatsUseCase(deckRepository, cardRepository, progressRepository);
  });
  ...
  it('should return correct statistics for a deck', async () => {
    ...
    expect(deckRepository.findByIdAndUserId).toHaveBeenCalledWith(deckId, userId);
