...
describe('DeleteDeckUseCase', () => {
  ...
  beforeEach(() => {
    useCase = new DeleteDeckUseCase(mockDeckRepository, mockCardRepository, mockProgressRepository);
    vi.useFakeTimers();
  ...
  describe('Unit Tests', () => {
    ...
    it('should throw DeckNotFoundError when deck is not found', async () => {
      ...
      await expect(promise).rejects.toThrow(`Deck with ID ${mockId} not found for user ${userId}`);
      expect(mockCardRepository.deleteByIds).not.toHaveBeenCalled();
  ...
  describe('BDD Scenarios', () => {
    ...
    it('Given a deck with cards, When execute is called, Then both cards and deck are deleted', async () => {
      ...
      expect(mockCardRepository.deleteByIds).toHaveBeenCalledWith(mockDeckWithCards.cards.map(card => card.id));
      expect(mockDeckRepository.delete).toHaveBeenCalledWith(mockId);
    });
