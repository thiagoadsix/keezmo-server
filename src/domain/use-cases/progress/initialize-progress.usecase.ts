...
export class InitializeProgressUseCase {
  ...
  public async execute({ cardId, deckId }: InitializeProgressRequest): Promise<InitializeProgressResponse> {
    console.log(`Starting for card=${cardId} in deck=${deckId}`);

    const existingProgress = await this.progressRepository.findByCardAndDeck(cardId, deckId);
