...
export class CreateCardsBatchUseCase {
  ...
  async execute(request: CreateCardsBatchRequest): Promise<CreateCardsBatchResponse> {
    const { deckId, userId, cards } = request;

    console.log(`Starting CreateCardsBatchUseCase for deckId: ${deckId} with ${cards.length} cards`);
