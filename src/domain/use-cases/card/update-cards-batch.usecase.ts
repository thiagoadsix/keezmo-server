...
interface UpdateCardsBatchRequest {
  deckId: string;
  userId: string;
  cards: CardToUpdate[];
}

type UpdateCardsBatchResponse = Card[];

export class UpdateCardsBatchUseCase {
  constructor(
  ...
  async execute(
    request: UpdateCardsBatchRequest
  ): Promise<UpdateCardsBatchResponse> {
    console.log(
