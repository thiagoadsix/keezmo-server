...
export class EndStudySessionUseCase {
  ...
  public async execute(request: EndStudySessionRequest): Promise<EndStudySessionResponse> {
    console.log('Ending study session', {
      sessionId: request.sessionId,
    });

    const studySession = await this.studySessionRepository.findById(request.sessionId);
